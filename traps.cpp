...
void trapTriangle(SDL_Renderer* renderer,std::vector<trapSpike>& trap,std::vector<ground_Class>& ground,int i)
{
    ...
    if(chances(gen)<50&&chances(gen)>10)
    {
        std::uniform_int_distribution<> distp(ground[i].x,ground[i].x+ground[i].w-100);
        ...
        if (ground[i].trapCount<4 )
        {
            ...
            trap.emplace_back(distp(gen), ground[i].y, ground[i].w, ground[i].h);
             if(trap.size()>1)
        {
            if(trap[trap.size()-1].x-trap[trap.size()-2].x<=50)
            {
                trap.back().x+=50;
            }
            break;
...
//         // if(trap.size()>0)
//         // {
//         //     if(trap[trap.size()-1].x-(distp(gen))<=50)
//         //     {
//         //         trap.emplace_back(distp(gen)+50, ground[i].y, ground[i].w, ground[i].h);
//         //         return ;
//         //     }
//         // }
